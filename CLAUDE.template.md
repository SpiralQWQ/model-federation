...
**触发**：路径含 `百度优化` 或 `baidu_optimization` 且涉及写操作。  
**流程**：代码改动发 QwenMax + GeminiFlash 独立审查 → 提交至少两方同意 → Blocker 强制第二轮讨论。  
**竞赛铁律**：单变量提交 · 改前备份(不用 git 回退) · 提交前检查清单 · 理论需实测验证 · 警惕无 CUDA 盲狙
...
| 版本 | 日期 | 变更 |
|------|------|------|
| V7.0 | 2026-07 | **架构重构**：627 行单体 → 核心规则(~120 行) + JIT 路由表 + spec/ 模块化目录；新增 GLM 质检门禁、量化数据采集、密钥遮蔽铁律；精简百度优化/三方协同为模板提示 |
| V6.0 | 2026-06 | 三方协同 V5.x、GLM 路由、四方会诊、百度优化铁律、计费播报 |
